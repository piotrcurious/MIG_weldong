...
void initiateArc() {
    ...
    // Read the retraction distance from the knob
    int retractAmount = analogRead(retractDistanceKnobPin);
    retractAmount = map(retractAmount, 0, 1023, 1, 100);  // Map knob value to stepper steps (adjustable range)

    // Stage 2: Current Introduction and Retraction
    analogWrite(pwmPin, pwmDutyCycle);  // Introduce current
    delay(10);  // Short delay to ensure current is established

    // Retract the wire by the adjustable amount
    moveStepper(-retractAmount, true);

    // Stage 3: Soft Start and Arc Stabilization
    while (!arcStable()) {
        currentTime = millis();
